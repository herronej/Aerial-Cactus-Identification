...
Data_NReduc = pca.transform(Data_NFlat)
'''
# LDA reduction
...
for i in range(F_n):
    ...
    model_name_DTree = 'Model_DTree_' + str(i+1) + '_LDA_rbf.model'
    Graph_name_DTree = 'Graph_DTree_' + str(i+1) + '_LDA_'
    # Decision Tree classfication
    clf_DTree = tree.DecisionTreeClassifier()
    clf_DTree.fit(DataN_tr,DataN_tr_y)
    score_DTree = clf_DTree.score(DataN_te,DataN_te_y)
    print("The score of PCA_DTree is : %f"%score_DTree)
