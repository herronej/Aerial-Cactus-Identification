...
# Reshape the image and PCA reduction
...
# Build BPNN
Data_in = tf.placeholder(dtype= tf.float32, shape=(None, 627), name="inputs")
...
BPnn = tf.layers.dense(Data_in, 30, activation=tf.nn.sigmoid, trainable=True, name='Input_L')
...
with tf.Session() as sess:
    ...
    for Ep_CNT in range(Epoch):
        ...
        Shuf_Data_Norm = Dada_NReduc[per, :]
